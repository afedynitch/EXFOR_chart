...
colors = dict(endf="#000000", jendl="#ff9900",
              tendl="#808000", psb="#00ff00", neucosma='#666699',
              talys='#00ccff', peanut="#9933ff")


def gen_cds(from_list, db_name, xoffs=.0, yoffs=.0):
    return dict(
...
def draw_mpl_rect(axis, xcol, ycol, xwidth, ywidth, source,
                  alpha, color, legend, picker=False):

    for i in range(len(list(source.items())[0][1])):
        axis.add_patch(patches.Rectangle(
            (source[xcol][i] - xwidth / 2., source[ycol][i] - ywidth / 2.),
            xwidth, ywidth, color=color, alpha=alpha, picker=picker
        ))

    proxy_artist = plt.Line2D(list(range(1)), list(range(1)),
                              color="white", marker='s',
                              markersize=14, alpha=alpha,
                              markerfacecolor=color)
    return (proxy_artist, legend)


def draw_mpl_cont(axis, xcol, ycol, xwidth, ywidth, source,
                  alpha, color, legend, picker=False):
    contour_width = 1.3
    for i in range(len(list(source.items())[0][1])):
        axis.add_patch(patches.Rectangle(
            (source[xcol][i] - xwidth / 2., source[ycol][i] - ywidth / 2.),
            xwidth, ywidth, edgecolor=color,
            alpha=alpha, facecolor='none',
            linewidth=contour_width, picker=picker
        ))
...
def draw_mpl_circle(axis, xcol, ycol, radius, source,
                    alpha, color, legend, picker=False):
    for i in range(len(list(source.items())[0][1])):
        axis.add_patch(patches.Circle(
            (source[xcol][i], source[ycol][i]),
            radius=radius, color=color, alpha=alpha,
            picker=picker
        ))
    proxy_artist = plt.Line2D(list(range(1)), list(range(1)),
...
class PointBrowser(object):
    ...
    def __init__(self, gs, fig, x4_dbm, nco_dbm, cds):
        ...
        self.lastpick = (0, 0)
        self.lastshow = (0, 0)
        self.text = self.chart_ax.text(0.8, 0.05, '',
                                       transform=self.chart_ax.transAxes,
                                       va='top', fontsize=18, weight='bold')
    ...
    def onpick(self, event):
        ...
        if (N, Z) == self.lastpick:
            return
        self.lastpick = (N, Z)
        print('{0}-{1} ({2})'.format(elem_names[Z], N + Z, nco_id))
        # distances = np.hypot(x - xs[event.ind], y - ys[event.ind])
        # indmin = distances.argmin()
        # dataind = event.ind[indmin]

        self.lastind = nco_id
        print('Updating.. from onpick')
        self.update()
    ...
    def draw_model_comparison(self, nco_id):
        for reac, ax in zip(['ABS', 'N', 'P', '2N'],
                            self.subpl_axes):
            x4_dbm.set_reaction(reac)
            select = x4_dbm.get_entries(nco_id)
            for k, e in six.iteritems(select):
        ...
        for db, label, c in [
            ...
            db.plot((0, nco_id), label=label, lw=2, color=c,
                    alpha=0.9, axes=self.abs_ax)
            db.plot((nco_id, nco_id - 100), label=label, lw=2, color=c,
                    alpha=0.9, axes=self.n_ax)
            db.plot((nco_id, nco_id - 101), label=label, lw=2, color=c,
                    alpha=0.9, axes=self.p_ax)
            db.plot((nco_id, nco_id - 200), label=label, lw=2, color=c,
                    alpha=0.9, axes=self.n2_ax)

        titles = ['{0}(g,abs)'.format(get_exfor_symb(nco_id)),
                  '{0}(g,n){1}'.format(
            get_exfor_symb(nco_id),
            get_exfor_symb(
                nco_id - 100)),
            '{0}(g,p){1}'.format(
            get_exfor_symb(nco_id),
...
with open(join(data_dir, 'neucosma_reduced_0_01.dat')) as f:
...
ww_important = pickle.load(open(join(data_dir,
                                     'ww_important_isotopes.ppd'), 'rb'))
...
nco_red_cds = gen_cds(convert_to_full_tuple(nco_dbm.neucos_abs_elems),
                      'NCO')
exfor_cds = gen_cds(convert_to_full_tuple(x4_dbm.get_nco_list_of_selection()),
                    'EXFOR, ABS')
exfor_all_cds = gen_cds(convert_to_full_tuple(
    list(set(np.loadtxt(join(data_dir, 'exfor_all.txt'))))), 'EXFOR, *')
nco_inj_cds = gen_cds(convert_to_full_tuple(ww_important[0]),
                      'NCO, injected')
nco1_cds = gen_cds(convert_to_full_tuple(ww_important[1]),
                   'NCO, Level 1')
nco2_cds = gen_cds(convert_to_full_tuple(ww_important[2]),
                   'NCO, Level 2')
endf_cds = gen_cds(convert_to_full_tuple(nco_dbm.endf_abs_elems),
                   'ENDF-B-VII.1', xoffs=-0.3, yoffs=.3)
jendl_cds = gen_cds(convert_to_full_tuple(nco_dbm.jendl_abs_elems),
                    'JENDL-PD-2004', xoffs=+0.3, yoffs=.3)
# tendl_cds = gen_cds(convert_to_full_tuple(nco_dbm.tendl_abs_elems),
#                     'TENDL-2015', xoffs=+0.3, yoffs=.3)
psb_cds = gen_cds(convert_to_full_tuple(nco_dbm.psb_abs_elems),
                  'PSB', xoffs=-0.3, yoffs=-0.3)
peanut_cds = gen_cds(convert_to_full_tuple(nco_dbm.peanut_abs_elems),
                  'PEANUT', xoffs=0.3, yoffs=-0.3)
peanut_cont_cds = gen_cds(convert_to_full_tuple(
...
def draw_iso_chart(chart_ax, logo=True):
    ...
    leg_list = []
    leg_list.append(draw_mpl_rect(chart_ax, "xpos", "ypos", xwidth, ywidth, source=selection_cds,
                                  alpha=0.0, color="w",
                                  legend='_nolegend_', picker=True))
    leg_list.append(draw_mpl_rect(chart_ax, "xpos", "ypos", xwidth, ywidth, source=nco_red_cds,
                                  alpha=1., color="#bfbfbf",
                                  legend='TALYS 1.8 + CRPropa 2'))
    leg_list.append(draw_mpl_rect(chart_ax, "xpos", "ypos", xwidth, ywidth, source=nco2_cds,
                                  alpha=0.6, color="#99ddff", legend='Astro, priority 2'))
    leg_list.append(draw_mpl_rect(chart_ax, "xpos", "ypos", xwidth, ywidth, source=nco1_cds,
                                  alpha=0.6, color="#3333ff", legend='Astro, priority 1'))
    leg_list.append(draw_mpl_rect(chart_ax, "xpos", "ypos", xwidth, ywidth, source=exfor_all_cds,
                                  alpha=0.9, color="#ffff66", legend=r'EXFOR, any'))
    leg_list.append(draw_mpl_rect(chart_ax, "xpos", "ypos", xwidth, ywidth, source=exfor_cds,
                                  alpha=0.9, color="#ff0066", legend=r'EXFOR, $\sigma_{\rm abs}$'))

    leg_list.append(draw_mpl_circle(chart_ax, "xpos", "ypos", 0.13, source=endf_cds,
                                    alpha=0.9, color=colors['endf'], legend='ENDF-B-VII.1'))
    leg_list.append(draw_mpl_circle(chart_ax, "xpos", "ypos", 0.13, source=jendl_cds,
                                    alpha=0.9, color=colors['jendl'], legend='JENDL-PD-2004'))
    # leg_list.append(draw_mpl_circle(chart_ax, "xpos", "ypos", 0.13, source=tendl_cds,
    #                 alpha=0.9, color=colors['tendl'], legend='TENDL-2015'))
    leg_list.append(draw_mpl_circle(chart_ax, "xpos", "ypos", 0.13, source=psb_cds,
                                    alpha=0.8, color=colors['psb'], legend='PSB'))
    leg_list.append(draw_mpl_circle(chart_ax, "xpos", "ypos", 0.13, source=peanut_cds,
                                    alpha=0.8, color=colors['peanut'], legend='PEANUT'))
    leg_list.append(draw_mpl_cont(chart_ax, "xpos", "ypos",
                                  xwidth + 0.02, ywidth + 0.02, source=peanut_cont_cds,
                                  alpha=.3, color=colors['peanut'], legend='_nolegend_'))
    leg_list.append(draw_mpl_cont(chart_ax, "xpos", "ypos",
                                  xwidth + 0.02, ywidth + 0.02, source=nco_inj_cds,
                                  alpha=1., color="#000000", legend='Astro, injected'))

    leg_list = [l for l in leg_list if l[1] != '_nolegend_']
    ...
    chart_ax.legend(*zip(*leg_list), loc='upper left', numpoints=1,
                    frameon=False)
    if logo:
        import matplotlib.image as image
        nco_logo = image.imread('../files/neucos-logo-noborder.png')
        chart_ax.imshow(nco_logo, aspect='auto',
                        extent=(25.2, 29.8, 0.5, 3.3), alpha=0.7, zorder=-1)
...
def start_static(show_logo, save):
    ...
    plt.tight_layout()
    if save:
        plt.savefig('../plots/exfor_chart.pdf', dpi=300)
        plt.savefig('../plots/exfor_chart.png', dpi=300)
    plt.show()
...
if __name__ == '__main__':
    ...
    parser.add_option("-i", "--interactive", action="store_true",
                      dest="interact", help="Interative panel mode.")
    parser.add_option("-c", "--chart", action="store_true",
                      dest="static_chart", help="Static chart mode for plotting.")
    parser.add_option("-s", "--save", action="store_true",
                      dest="save_chart", help="Save chart as pdf and png.")
    parser.add_option("-l", "--logo", action="store_true",
                      dest="show_logo", help="Show logo on chart.")
